...
{{-- Load Bebas Neue — benang merah font dengan member-side card --}}
...
<style>
    ...
    .stat-label { font-size: 10px; font-weight: 800; color: var(--muted); text-transform: uppercase; letter-spacing: .1em; margin: 0 0 6px; }
    ...
    /* ── Table card ── */
    .mem-panel { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); box-shadow: var(--shadow); overflow: hidden; }
    .mem-panel-head { padding: 13px 16px; border-bottom: 1px solid var(--border); background: #fafafa; display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; }
    ...
    /* ── Search ── */
    .mem-search { display: flex; align-items: center; gap: 8px; background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 0 12px; height: 34px; transition: border-color .15s, box-shadow .15s; }
    .mem-search:focus-within { border-color: rgba(239,68,68,.4); box-shadow: 0 0 0 3px rgba(239,68,68,.08); }
    .mem-search i { font-size: 11px; color: var(--muted); }
    .mem-search input { border: none; outline: none; background: transparent; font-size: 12px; color: var(--text); font-family: 'Outfit', sans-serif; width: 200px; }
    .mem-search input::placeholder { color: #bbb; }

    .export-btn { display: inline-flex; align-items: center; gap: 6px; height: 34px; padding: 0 14px; border-radius: var(--radius); background: rgba(22,163,74,.08); color: #166534; border: 1px solid rgba(22,163,74,.2); font-size: 12px; font-weight: 700; font-family: 'Outfit', sans-serif; text-decoration: none; transition: background .15s, color .15s; }
    ...
    .mem-table thead th { padding: 10px 14px; font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: .1em; color: var(--muted); border-bottom: 1px solid var(--border); background: #fafafa; white-space: nowrap; text-align: left; }
    .mem-table thead th:last-child { text-align: right; }
    .mem-table tbody td { padding: 11px 14px; font-size: 13px; color: var(--text); border-bottom: 1px solid rgba(0,0,0,.04); vertical-align: middle; }
    .mem-table tbody tr:last-child td { border-bottom: none; }
    .mem-table tbody tr:hover td { background: #fafafa; }

    .mem-avatar { width: 34px; height: 34px; border-radius: var(--radius); background: linear-gradient(135deg, var(--red), #991b1b); display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 800; color: #fff; flex-shrink: 0; }
    ...
    /* ── Pills ── */
    .pill { display: inline-flex; align-items: center; gap: 4px; padding: 3px 8px; border-radius: var(--radius); font-size: 10px; font-weight: 700; white-space: nowrap; }
    ...
    /* ── Action buttons ── */
    .act-btn { width: 30px; height: 30px; border-radius: var(--radius); display: inline-flex; align-items: center; justify-content: center; border: 1px solid var(--border); background: rgba(0,0,0,.03); cursor: pointer; transition: background .15s, border-color .15s; flex-shrink: 0; }
    ...
    /* Empty state */
    .empty-state { padding: 52px 24px; text-align: center; display: flex; flex-direction: column; align-items: center; gap: 8px; }
    .empty-icon { width: 42px; height: 42px; border-radius: var(--radius); background: rgba(0,0,0,.04); display: flex; align-items: center; justify-content: center; margin-bottom: 4px; }
    ...
    /* ══════════════════════════════
       MODAL BASE
    ══════════════════════════════ */
    .ub-modal { display: none; position: fixed; inset: 0; z-index: 200; align-items: center; justify-content: center; padding: 16px; }
    .ub-modal.open { display: flex; }
    .ub-backdrop { position: absolute; inset: 0; background: rgba(0,0,0,.35); backdrop-filter: blur(4px); }
    .ub-box { position: relative; z-index: 1; background: var(--surface); border: 1px solid var(--border); width: 100%; border-radius: var(--radius); box-shadow: 0 8px 40px rgba(0,0,0,.14); overflow: hidden; animation: alertIn .2s ease; max-height: 90vh; overflow-y: auto; }
    @keyframes alertIn { from { opacity:0; transform:scale(.95) translateY(-6px); } to { opacity:1; transform:scale(1) translateY(0); } }

    .ub-head { display: flex; align-items: center; justify-content: space-between; padding: 16px 18px 12px; border-bottom: 1px solid var(--border); position: sticky; top: 0; background: var(--surface); z-index: 2; }
    .ub-title { font-size: 14px; font-weight: 700; color: var(--text); margin: 0 0 2px; }
    .ub-sub   { font-size: 11px; color: var(--muted); margin: 0; }
    .ub-close { width: 28px; height: 28px; border-radius: var(--radius); background: rgba(0,0,0,.04); border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: background .15s; }
    .ub-close:hover { background: rgba(0,0,0,.08); }
    .ub-close i { font-size: 11px; color: var(--muted); }
    .ub-foot { display: flex; justify-content: flex-end; gap: 8px; padding: 12px 18px; border-top: 1px solid var(--border); background: #fafafa; position: sticky; bottom: 0; }

    /* ══════════════════════════════
       DIGITAL MEMBER CARD
       — benang merah: identik dgn member-side —
    ══════════════════════════════ */
    .digi-card {
        margin: 14px;
        border-radius: 12px;
    ...
    /* Status variants — IDENTIK dengan member-side */
    .digi-card.card-active {
        background: linear-gradient(135deg, #0e1a14 0%, #111111 60%);
        border-left: 3px solid #10b981;
    }

    .digi-card.card-inactive {
        background: linear-gradient(135deg, #1a0e0e 0%, #111111 60%);
        border-left: 3px solid #ef4444;
    }

    /* Shimmer — IDENTIK dengan member-side */
    .card-shimmer {
        position: absolute;
        inset: 0;
        border-radius: 12px;
    ...
    /* Decorative circles — IDENTIK dengan member-side */
    .digi-deco1 {
        position: absolute;
    ...
    .card-active  .digi-deco1 { background: radial-gradient(circle, rgba(16,185,129,.15) 0%, transparent 70%); }
    .card-inactive .digi-deco1 { background: radial-gradient(circle, rgba(239,68,68,.15) 0%, transparent 70%); }
    ...
    /* Gym name — font Bebas Neue IDENTIK dengan member-side */
    .digi-gym-name {
    ...
    .digi-gym-name span { color: #ef4444; }

    /* Status badge — IDENTIK dengan member-side */
    .digi-status-badge {
        padding: 3px 10px;
        border-radius: 20px;
        font-size: 9px;
    ...
    .digi-status-badge.active {
        background: rgba(16,185,129,.15);
        color: #10b981;
        border: 1px solid rgba(16,185,129,.3);
    }

    .digi-status-badge.inactive {
        background: rgba(239,68,68,.15);
        color: #ef4444;
        border: 1px solid rgba(239,68,68,.3);
    }

    /* Chip ornamen — IDENTIK dengan member-side */
    .digi-chip {
    ...
    /* Nama member — font Bebas Neue IDENTIK dengan member-side */
    .digi-mname {
        font-family: 'Bebas Neue', sans-serif;
        font-size: 26px;
        letter-spacing: .06em;
    ...
    .digi-fl { font-size: 9px; text-transform: uppercase; color: rgba(255,255,255,.4); font-weight: 700; letter-spacing: .1em; margin-bottom: 3px; }

    /* Kode member — warna oranye monospace IDENTIK dengan member-side */
    .digi-fv {
        font-size: 12px;
        font-weight: 800;
    font-family: monospace;
    ...
    /* QR icon area */
    .digi-qr-wrap { display: flex; align-items: center; gap: 8px; }

    .digi-qr-icon { font-size: 22px; }
    .digi-qr-icon.active  { color: #10b981; }
    .digi-qr-icon.inactive { color: rgba(255,255,255,.25); }
    ...
    .vm-icon { width: 18px; height: 18px; border-radius: 50%; background: rgba(0,0,0,.04); display: flex; align-items: center; justify-content: center; transition: background .15s; }
    ...
    .ext-row { display: flex; align-items: center; justify-content: space-between; padding: 7px 0; border-bottom: 1px solid rgba(0,0,0,.04); }
    ...
    .btn-edit-full { display: flex; align-items: center; justify-content: center; gap: 6px; width: 100%; padding: 10px; border-radius: var(--radius); background: var(--red); color: #fff; font-size: 12px; font-weight: 700; border: none; cursor: pointer; font-family: 'Outfit', sans-serif; margin-top: 14px; transition: background .15s; }
    ...
    .field-input { width: 100%; border: 1px solid var(--border); border-radius: var(--radius); padding: 8px 12px; font-size: 13px; color: var(--text); outline: none; font-family: 'Outfit', sans-serif; background: #fafafa; transition: border-color .15s, box-shadow .15s; box-sizing: border-box; }
    .field-input:focus { border-color: rgba(239,68,68,.4); box-shadow: 0 0 0 3px rgba(239,68,68,.08); background: var(--surface); }

    .btn-cancel { padding: 7px 14px; border-radius: var(--radius); background: rgba(0,0,0,.04); color: var(--muted); border: 1px solid var(--border); font-size: 12px; font-weight: 700; font-family: 'Outfit', sans-serif; cursor: pointer; transition: background .15s; }
    .btn-cancel:hover { background: rgba(0,0,0,.08); color: var(--text); }
    .btn-save { padding: 7px 16px; border-radius: var(--radius); background: var(--red); color: #fff; border: none; font-size: 12px; font-weight: 700; font-family: 'Outfit', sans-serif; cursor: pointer; transition: background .15s; }
    ...
    <div class="mem-panel-head">
        ...
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
            <form method="GET" action="{{ route('admin.data.members') }}" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:0;">
                <select name="status" onchange="this.form.submit()" style="padding:6px 12px;border-radius:6px;border:1px solid #ccc;font-size:13px;outline:none;background:#fff;">
                ...
                <select name="package_filter" onchange="this.form.submit()" style="padding:6px 12px;border-radius:6px;border:1px solid #ccc;font-size:13px;outline:none;background:#fff;">
                    ...
                    <td>
                        <div style="display:flex;align-items:center;justify-content:flex-end;gap:5px;">
                            ...
                            <button type="button" class="act-btn act-btn-detail" title="Detail Member"
                                ...
                                data-member-code="{{ $m->member_code ?? 'NON MEMBER' }}"
                                data-activation="{{ $m->is_active_member ? 'AKTIF' : 'NON AKTIVASI' }}"
                                data-activation-bool="{{ $m->is_active_member ? '1' : '0' }}"
                                data-package="{{ $membership ? addslashes($membership->package_name) : 'Belum Ada Paket' }}"
                                data-expired="{{ $membership ? \Carbon\Carbon::parse($membership->end_date)->format('d M Y') : '-' }}"
                                data-package-status="{{ !$hasPackage ? 'Belum Ada Paket' : ($packageExpired ? 'Expired' : 'Aktif') }}"
                                data-pt="{{ $hasPT ? $hasPT.' Sesi Aktif' : 'Tidak Ada' }}"
                                data-created="{{ $m->created_at->format('d M Y') }}"
...
<div class="ub-modal" id="detailModal">
    ...
    <div class="ub-box" style="max-width:420px;">
        ...
        {{-- ══ DIGITAL CARD — benang merah visual ══ --}}
        <div class="digi-card card-inactive" id="digiCard">
...
{{-- Top row: gym name + status badge --}}
<div class="digi-top">
    <div class="digi-gym-name">
      {{ $settings['gym_name'] ?? 'Nama Gym Belum Diatur' }}
    </div>
</div>
            {{-- Chip + nama --}}
            <div style="position:relative;">
                <div class="digi-chip"></div>
                <div class="digi-mname" id="dc_name">—</div>
            </div>

            {{-- Footer card --}}
            <div class="digi-footer">
                <div>
                    <div class="digi-fl">Kode Member</div>
                    <div class="digi-fv" id="dc_code">—</div>
                </div>
                <div class="digi-qr-wrap">
                    <div style="text-align:right;">
                        <div class="digi-fl">Berlaku S/D</div>
                        <div class="digi-fv2" id="dc_expired">—</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="detail-rows">
            ...
            <div class="ext-details" id="ext_details">
                ...
                <div class="ext-row">
                    <span class="ext-key">Status Aktivasi</span>
                    <span class="ext-val" id="ed_activation">—</span>
                </div>
...
<script>
    ...
    /* ─── openDetail: pakai data-* attribute — aman dari karakter khusus ─── */
    function openDetail(btn) {
        ...
        const m = {
            ...
            member_code:    btn.dataset.memberCode,
            activation:     btn.dataset.activation,
            isActive:       btn.dataset.activationBool === '1',
            package:        btn.dataset.package,
            expired:        btn.dataset.expired,
            package_status: btn.dataset.packageStatus,
            pt:             btn.dataset.pt,
        ...
        /* ── Update card visual (benang merah: identik member-side) ── */
        const card  = document.getElementById('digiCard');
        const qr    = document.getElementById('dc_qr');

        /* ── Isi konten card ── */
document.getElementById('dc_name').textContent    = m.name.toUpperCase();
       
        document.getElementById('dc_code').textContent    = m.member_code;
        ...
        document.getElementById('ed_code').textContent       = m.member_code;
        document.getElementById('ed_activation').textContent = m.activation;
        document.getElementById('ed_activation').style.color = m.isActive ? '#16a34a' : '#ef4444';
        ...
        document.getElementById('btn_edit_from_detail').onclick = function() {
            closeDetail();
            setTimeout(() => openEdit(m.id, m.name, m.whatsapp), 130);
        };
